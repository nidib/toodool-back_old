...
const toodoolServicesQueryStrings = {
	...
	selectToodoolByUserAndTitleQueryString: `SELECT * FROM ${toodoolTable} WHERE user_id = $1 AND title = $2`,
	completeToodoolQueryString: `UPDATE ${toodoolTable} SET completed = $1 WHERE user_id = $2 AND id = $3`,
	getToodolsByUserQueryString: `SELECT replace(id::text, '-', '') AS oid, completed, title, description FROM ${toodoolTable} WHERE user_id = $1 ORDER BY created_at DESC`,
	getToodoolByUserAndIdQueryString: `SELECT replace(id::text, '-', '') as oid, completed, title, description FROM ${toodoolTable} WHERE user_id = $1 and id = $2`,
...
const userServicesQueryStrings = {
	selectUserByUsernameQueryString: `SELECT replace(id::text, '-', '') AS id, password FROM ${userTable} WHERE username = $1`,
